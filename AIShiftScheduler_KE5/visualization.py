...
# from AIShiftScheduler_KE5.model import * ## cause cyclic import
...
@patch
def plot_records(self:Visualization, df, df_non, pars=defaultdict(str)):
    ## add capacities
    df = copy(df)
    ...
    for i,an in enumerate(cf.aNAMES):
      df[f'cap_{an}'] = \
        df.apply(lambda row: \
          cf.get_capacities(dow=row['dow'])[i], axis=1)
    ...
    xi = 0
    for i,ab in enumerate(cf.abNAMES):
      ...
      for j in range(df.shape[0]//cf.SLOTS_PER_DAY): axs[xi+i].axvline(x=(j+1)*cf.SLOTS_PER_DAY, color='grey', ls=':')
      for j in range(df.shape[0]//T): axs[xi+i].axvline(x=(j+1)*T, color='black', ls='-.')

    xi = n_ab
    for i,b in enumerate(cf.bNAMES):
    ...
    for j in range(df.shape[0]//cf.SLOTS_PER_DAY): axs[xi].axvline(x=(j+1)*cf.SLOTS_PER_DAY, color='grey', ls=':')
    ...
    xi = n_ab + n_b + 3
    for i,a in enumerate(cf.aNAMES):
      axs[xi+i].set_ylim(auto=True); axs[xi+i].spines['top'].set_visible(False); axs[xi+i].spines['right'].set_visible(True); axs[xi+i].spines['bottom'].set_visible(False)
    ...
    xi = n_ab + n_b + 3 + n_a
    Ucum_cols = ['Ucum_'+bn for bn in cf.bNAMES]
...
@patch
def plot_demand_sources(self:Visualization, df, pars=defaultdict(str)):
      n_b = len(cf.bNAMES)
      n_charts = n_b + 1 + 1 + 1
      ylabelsize = 14
      mpl.rcParams['lines.linewidth'] = 1.2
...
@patch
def setup_plot_labels(self:Visualization):
    RAvail_t_labels = ['RAvail_t_'+an for an in cf.aNAMES]
    RCumSlots_t_labels = ['RCumSlots_t_'+an for an in cf.aNAMES]
    RCumMerits_t_labels = ['RCumMerits_t_'+an for an in cf.aNAMES]
    ## RContSlots_t_labels = ['RContSlots_t_'+an for an in cf.aNAMES]
    RComplete_t_labels = ['RComplete_t_'+an for an in cf.aNAMES]
    DSlot_t_labels = ['DSlot_t_'+rt for rt in cf.RESOURCE_TYPES]
    DBusy_t_labels = ['DBusy_t_'+rt for rt in cf.RESOURCE_TYPES]
    DVolume_t_labels = ['DVolume_t_'+rt for rt in cf.RESOURCE_TYPES]
    DRevenue_t_labels = ['DRevenue_t_'+rt for rt in cf.RESOURCE_TYPES]
    DMax_t_labels = ['DMax_t_'+rt for rt in cf.RESOURCE_TYPES]
    B_t_label = ['B_t']
    V_t_label = ['V_t']
    N_t_label = ['N_t']
    xAlloc_t_labels = ['Allocd_t_'+abn for abn in cf.abNAMES]
    labels = ['piName', 'theta', 'l'] + \
      ...
      V_t_label + \
      N_t_label + \
      ['Ucum_'+rt for rt in cf.RESOURCE_TYPES] + \
      ['Ucum_Total'] + \
      ['Ccum_CumSlots'] + \
      ['Ccum_SickProb'] + \
      ['Ccum_CumMerits'] + \
